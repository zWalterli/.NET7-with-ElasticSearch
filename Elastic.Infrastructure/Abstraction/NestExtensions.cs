...
namespace Elastic.Infrastructure.Abstraction
{
    public static class NestExtensions
    {
        ...
        {
            List<IndexActors> list = GenerateListOfActors(1000000);
            /*
        ...
        private static List<IndexActors> GenerateListOfActors(int count)
        {
            List<IndexActors> actorsList = new List<IndexActors>();
            for (int i = 0; i < count; i++)
            {
                actorsList.Add(new IndexActors
                {
                    Id = Guid.NewGuid().ToString(),
                    RegistrationDate = DateTime.Now,
                    BirthDate = RandomBirthDate(),
                    Age = RandomAge(),
                    TotalMovies = RandomTotalMovies(),
                    Name = "Actor Name " + i,
                    Description = "Actor Description " + i
                });
            }
            return actorsList;
        }
        ...
        private static DateTime RandomBirthDate()
        {
            ...
            int day = random.Next(1, 29); // Limiting days to avoid invalid dates
        ...
        private static int RandomTotalMovies()
        {
            Random random = new Random();
